...
public class SecureCommand extends AbstractCommand {
	...
	public String execute(Database database) {
		...
		if (!currentUser.isAuthenticated()) {
			throw new VectorizationException("User is not authenticated");
		}
		String name = "*";
		if(database != null) name = database.getName();
		if(!currentUser.isPermitted(name+":" + delegate.getPermissionLevel())){
			throw new VectorizationException(currentUser.getPrincipal() + " is not authorized");
		}
